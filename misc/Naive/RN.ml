...
let permute var = if var mod 6 = pk1 then
                      ...
                      3*(var / 6) + man.field_max 
                    else if var mod 6 = pk3 then
                      3*(var / 6) + man.field_max + 1
                    else if var mod 6 = pk4 then
                      3*(var / 6) + man.field_max + 2
                    else
                      raise (Invalid_argument "re_ordering: variable not in the expected range")
                    in MLBDD.permutef permute bdd

let back_ordering (man:man) (pk1:pk) (pk2:pk) (pk3:pk) (pk4:pk) (bdd:MLBDD.t):MLBDD.t=
  let permute var = if var < man.field_max then
                      6 * var + pk1
                    else if (var - man.field_max) mod 3 = 0 then
                       (var - man.field_max)/ 3 * 6 + pk2
                    else if (var - man.field_max) mod 3 = 1 then
                      (var -  man.field_max) / 3 * 6 + pk3
                    else if (var - man.field_max) mod 3 = 2 then
                      (var -  man.field_max) / 3 * 6 + pk4
                    else 
                      raise (Invalid_argument "back_ordering: variable not in the expected range")  
                    in MLBDD.permutef permute bdd    
(* pk1: x, pk2:x', pk3:y, pk4:y'*)
...
let splitting_bdd (man:man)(pk1:pk)(pk2:pk)(pk3:pk)(pk4:pk) (bdd:MLBDD.t):  BSet.t =
  ...
  let rec loop (bdd:MLBDD.t):BSet.t =
    ...
    with Not_found ->
      ...
      bset
  in
    BSet.map (fun bdd -> back_ordering man pk1 pk2 pk3 pk4 bdd) (loop (re_ordering man pk1 pk2 pk3 pk4 bdd))
