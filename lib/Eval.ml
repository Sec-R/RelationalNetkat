...
let init_man (nodes:Yojson.Basic.t) (edges:Yojson.Basic.t) (protocols:Yojson.Basic.t) (interfaces:Yojson.Basic.t): man =
  ...
  let length =   nodes_length + vrf_length + protocols_length + 32 + 16 + 32 + 16 + 1 in
  {
    ...
    interface = DStringMap.mapi 
      (fun (node,_) (in_filter, out_filter) -> (parse_protocol_filter node in_filter protocols nodesmap vrf_length protocol_map, parse_protocol_filter node out_filter protocols nodesmap vrf_length protocol_map)
...
let parse_rela_local_routing_table (rela:Yojson.Basic.t) (man:man) : NK.t * NK.t =
  ...
  in
    let before_table = aux (Binary (False,False)) (rela |> member "graphBefore" |> member "nodeToOutEdgesMap") in
    let after_table = aux (Binary (False,False)) (rela |> member "graphAfter" |> member "nodeToOutEdgesMap") in
