...
(* This defintion is exactly same as the Rela file rela/language/regularir/regularir.py,
truncated to the relevant part for the parser.
*)
type path = 
  | PSymbol of string
...
let header_placement (header:header) (man: man) : int =
  ...
  | Tunnel -> 96 + man.nodes_length + man.vrf_length + man.protocols_length
...
let partial_header_placement (header:header) (node_length: int) (vrf_length: int) (protocol_length: int) : int =
  ...
  | Tunnel -> 96 + (node_length + vrf_length + protocol_length)
...
let insert_tunnel (node:string) (interface:string) (tunnel_map:(int DStringMap.t)): int DStringMap.t =
  let tunnel_number = (DStringMap.cardinal tunnel_map)+1 in
  let key = (node, interface) in
  if DStringMap.mem key tunnel_map then
    tunnel_map
  else
    DStringMap.add key tunnel_number tunnel_map

let parse_ipsec_tunnels (ipsec_tunnels:Yojson.Basic.t): (int DStringMap.t)*(int DStringMap.t) =
  let rec aux (acc1, acc2) = function
    | `Assoc [] -> (acc1, acc2)
    | `Assoc ((_,entry)::xs) ->
      let encrypt_node = entry |> member  "Tunnel_Interface" |> member "hostname" |> to_string in
      let encrypt_interface = entry |> member  "Tunnel_Interface" |> member "interface" |> to_string in
      let decrypt_node = entry |> member  "Remote_Tunnel_Interface" |> member "hostname" |> to_string in
      let decrypt_interface = entry |> member  "Remote_Tunnel_Interface" |> member "interface" |> to_string in
      let acc1 = insert_tunnel encrypt_node encrypt_interface acc1 in
      let acc2 = insert_tunnel decrypt_node decrypt_interface acc2 in
      aux (acc1, acc2) (`Assoc xs)
    | _ -> failwith "Unexpected JSON format for IPsec tunnels, please check the json generated by the batfish is in orient format of 'index'"  
  in
  aux (DStringMap.empty, DStringMap.empty) ipsec_tunnels

let get_incoming_tunnel (node:string) (interface:string) (tunnel_map:(int DStringMap.t))(node_length: int) (vrf_length: int) (protocol_length: int) : pkr =
  let tunnel_id = try (DStringMap.find (node, interface) tunnel_map) with _ -> 0 in
  let tunnel_length = length_of_int ((DStringMap.cardinal tunnel_map) + 1) in
  let tunnel_header = partial_header_placement Tunnel node_length vrf_length protocol_length in
  let tunnel_pred = binary_to_pred tunnel_header tunnel_length (tunnel_length - 1) tunnel_id in
  let tunnel_pkr = binary_to_pkr tunnel_header tunnel_length (tunnel_length - 1) 0 in
  Comp (AndP (Binary (True, tunnel_pred), Id), tunnel_pkr)

let get_outgoing_tunnel (node:string) (interface:string) (tunnel_map:(int DStringMap.t))(node_length: int) (vrf_length: int) (protocol_length: int) : pkr =
  let tunnel_id = try (DStringMap.find (node, interface) tunnel_map) with _ -> 0 in
  let tunnel_length = length_of_int ((DStringMap.cardinal tunnel_map) + 1) in
  let tunnel_header = partial_header_placement Tunnel node_length vrf_length protocol_length in
  let tunnel_pred = binary_to_pred tunnel_header tunnel_length (tunnel_length - 1) 0 in
  let tunnel_pkr = binary_to_pkr tunnel_header tunnel_length (tunnel_length - 1) tunnel_id in
  Comp (AndP (Binary (True, tunnel_pred), Id), tunnel_pkr)

let get_no_tunnel (tunnel_length:int) (node_length: int) (vrf_length: int) (protocol_length: int) : pkr =
  let tunnel_header = partial_header_placement Tunnel node_length vrf_length protocol_length in
  let tunnel_pred = binary_to_pred tunnel_header tunnel_length (tunnel_length - 1) 0 in
  AndP (Binary (True, tunnel_pred), Id)  


let init_man (nodes:Yojson.Basic.t) (edges:Yojson.Basic.t) (protocols:Yojson.Basic.t) (interfaces:Yojson.Basic.t) (ipsec_tunnels:Yojson.Basic.t option): man =
  ...
  let (tunnel_map1, tunnel_map2) = match ipsec_tunnels with
    | Some json -> parse_ipsec_tunnels json
    | None -> (DStringMap.empty, DStringMap.empty)
  in
  let tunnel_length = length_of_int ((DStringMap.cardinal tunnel_map1) + 1) in
  let length =  nodes_length + vrf_length + protocols_length + tunnel_length + 32 + 16 + 32 + 16 + 1 in
  {
    nodes = nodesmap;
    edges = parse_edges_to_map edges;
    protocols = protocol_map;
    interface = DStringMap.mapi 
      (fun (node,interface) (in_filter, out_filter) -> 
        let incoming_tunnel = get_incoming_tunnel node interface tunnel_map2 nodes_length vrf_length protocols_length in
        let outgoing_tunnel = get_outgoing_tunnel node interface tunnel_map1 nodes_length vrf_length protocols_length in
        (Comp (pred_to_filter (parse_protocol_filter node in_filter protocols nodesmap vrf_length protocol_map),incoming_tunnel),
        Comp (pred_to_filter (parse_protocol_filter node out_filter protocols nodesmap vrf_length protocol_map),outgoing_tunnel)
    ...
    interfaces_to_tunnels = (tunnel_map1, tunnel_map2);
...
(* Debugger *) 
let compare_interface (man1:man) (man2:man) : unit =
  let interfaces1 = man1.interface in
  let interfaces2 = man2.interface in
  let pk0 = 0 in
  let pk1 = 1 in
  let man = RN.init_man (man1.length + man2.length) (man1.length + man2.length) in
  DStringMap.iter (fun (node, interface) (in_filter1, out_filter1) ->
    try
      let (in_filter2, out_filter2) = DStringMap.find (node, interface) interfaces2 in
      let bdd_in1 = RN.compile_pkr_bdd man pk0 pk1 in_filter1 in
      let bdd_in2 = RN.compile_pkr_bdd man pk0 pk1 in_filter2 in
      let bdd_out1 = RN.compile_pkr_bdd man pk0 pk1 out_filter1 in
      let bdd_out2 = RN.compile_pkr_bdd man pk0 pk1 out_filter2 in
      if not (MLBDD.equal bdd_in1 bdd_in2) then
        Printf.printf "Incoming filter mismatch for %s %s: %d vs %d\n" node interface (MLBDD.id bdd_in1) (MLBDD.id bdd_in2);
      if not (MLBDD.equal bdd_out1 bdd_out2) then
        Printf.printf "Outgoing filter mismatch for %s %s: %d vs %d\n" node interface (MLBDD.id bdd_out1) (MLBDD.id bdd_out2);
    with Not_found ->
      Printf.printf "Interface %s %s not found in second man\n" node interface
  ) interfaces1

let compare_interface_with_edgeMap (man1:man) (man2:man) : unit =
  let edges1 = man1.edges in
  let interfaces1 = man1.interface in
  let interfaces2 = man2.interface in
  let pk0 = 0 in
  let pk1 = 1 in
  ...
  DStringMap.iter (fun (node1, interface1) (node2, interface2) ->
    try
      let bdd1 = RN.compile_pkr_bdd man pk0 pk1
        (Comp (snd (DStringMap.find (node1, interface1) man1.interface),
                find_next_loc_filter node1 interface1 man1)) in
      let bdd2 = RN.compile_pkr_bdd man pk0 pk1
        (Comp (snd (DStringMap.find (node2, interface2) man2.interface),
                find_next_loc_filter node2 interface2 man2)) in
      if not (MLBDD.equal bdd1 bdd2) then
        Printf.printf "Edge filter mismatch for %s %s: %d vs %d\n" node1 interface1 (MLBDD.id bdd1) (MLBDD.id bdd2);          
      let (_, out_filter1) = DStringMap.find (node1, interface1) interfaces1 in
      let (in_filter1, _) = DStringMap.find (node2, interface2) interfaces1 in
      let ebdd_1 = RN.compile_pkr_bdd man pk0 pk1 (Comp (out_filter1, in_filter1)) in
      let (_, out_filter2) = DStringMap.find (node1, interface1) interfaces2 in
      let (in_filter2, _) = DStringMap.find (node2, interface2) interfaces2 in
      let ebdd_2 = RN.compile_pkr_bdd man pk0 pk1 (Comp (out_filter2, in_filter2)) in
      if not (MLBDD.equal ebdd_1 ebdd_2) then
        Printf.printf "Edge filter mismatch for %s %s: %d vs %d\n" node1 interface1 (MLBDD.id ebdd_1) (MLBDD.id ebdd_2);
      with Not_found ->
        Printf.printf "Edge %s %s not found in second man\n" node1 interface1
  ) edges1


let init_man_disable_tunnel (nodes:Yojson.Basic.t) (edges:Yojson.Basic.t) (protocols:Yojson.Basic.t) (interfaces:Yojson.Basic.t) (ipsec_tunnels:Yojson.Basic.t option): man =
  let nodesmap = parse_nodes_to_map nodes in
  let protocol_map = parse_protocols_to_map protocols in
  let vrf_map = parse_vrf_to_map protocols in
  let nodes_length = length_of_int (StringMap.cardinal nodesmap) in
  ...
  let length =  nodes_length + vrf_length + protocols_length + tunnel_length + 32 + 16 + 32 + 16 + 1 in
  {
    ...
    protocols = protocol_map;
    interface = DStringMap.mapi 
      (fun (node,_) (in_filter, out_filter) -> 
        let incoming_tunnel = get_no_tunnel tunnel_length nodes_length vrf_length protocols_length in
        let outgoing_tunnel = get_no_tunnel tunnel_length nodes_length vrf_length protocols_length in
        (Comp (pred_to_filter (parse_protocol_filter node in_filter protocols nodesmap vrf_length protocol_map),incoming_tunnel),
        Comp (pred_to_filter (parse_protocol_filter node out_filter protocols nodesmap vrf_length protocol_map),outgoing_tunnel)
    )) (parse_interfaces_to_map interfaces);
    vrf = vrf_map;
            ...
            aux (re_routing_list @ [(route_ip, And (Neg filter,new_filter))]) actions next_mask_list next_cur next_filter 
              next_mask_filter next_routing_table next_pending_actions xs
                  ...
                  if String.equal interface "Loopback0"
                    then Empty
                    (* Setting the loopback interface to empty, as it is not used in the routing table *) 
                    (* If one want to include it, set the following to True*)
                    (* post_filter_pkr (snd (DStringMap.find (loc, interface) man.interface))
                  ...
                  else try (Comp (snd (DStringMap.find (loc, interface) man.interface),
                    Comp (find_next_loc_filter loc interface man,Comp (parse_location_to_pkr (find_next_loc loc interface man) false man,
...
let parse_start_loc_to_pkr (man:man) (start_list:string list) : pkr =
  if List.is_empty start_list then
    Id
  else
...
let parse_end_loc_to_pkr (man:man) (end_list:string list) : pkr =
  if List.is_empty end_list then
    Id
  else
  List.fold_left (fun acc loc ->
  try 
...
let parse_encrypted_loc_to_pred (man:man) (encrypted_list:string list) : pred =
  List.fold_left (fun acc loc ->
    let encrypted_loc = parse_location_to_pred loc false man in
    Or (acc, encrypted_loc)
  ) False encrypted_list


let compare_node (man1:man) (man2:man) (table:Yojson.Basic.t) : unit =
  let pk0 = 0 in
  let pk1 = 1 in
  let man = RN.init_man (man1.length + man2.length) (man1.length + man2.length) in
  (match table with
  | `Assoc list -> list
  | _ -> [])
  |> (List.iter (fun (loc, routing_table) ->
    let routing_table = routing_table |> to_list in
    let action1 = parse_local_routing_table loc routing_table man1 in
    let action2 = parse_local_routing_table loc routing_table man2 in
    let bdd1 = RN.compile_pkr_bdd man pk0 pk1 action1 in
    let bdd2 = RN.compile_pkr_bdd man pk0 pk1 action2 in
    if not (MLBDD.equal bdd1 bdd2) then
      Printf.printf "Node filter mismatch for %s : %d vs %d\n" loc (MLBDD.id bdd1) (MLBDD.id bdd2);
  ));
  let routing_table1 = parse_global_routing_table table man1 in
  let routing_table2 = parse_global_routing_table table man2 in
  let bdd1 = RN.compile_pkr_bdd man pk0 pk1 routing_table1 in
  let bdd2 = RN.compile_pkr_bdd man pk0 pk1 routing_table2 in
  if not (MLBDD.equal bdd1 bdd2) then
    Printf.printf "Global routing table mismatch: %d vs %d\n" (MLBDD.id bdd1) (MLBDD.id bdd2)

let encrypt_to_pred (man:man) (tunnel_id:int):pred =
  let tunnel_length = man.tunnel_length in
  let tunnel_header = header_placement Tunnel man in
  binary_to_pred tunnel_header tunnel_length (tunnel_length -1) tunnel_id

let encrypt_to_pkr (man:man) (tunnel_id:int):pkr =
  let tunnel_length = man.tunnel_length in
  let tunnel_header = header_placement Tunnel man in
  binary_to_pkr tunnel_header tunnel_length (tunnel_length -1) tunnel_id

let encrypt_network_from_routing_table (routing_table:pkr) (man:man) (encrypted_list:string list): pkr =
  let encrypted_loc = parse_encrypted_loc_to_pred man encrypted_list in
  let encrypt_network = encrypt_to_pkr man 1 in
  let decrypt_network = encrypt_to_pkr man 0 in
  let encrypt_pred = encrypt_to_pred man 1 in
  let decrypt_pred = encrypt_to_pred man 0 in
  let encrypt_pkr = AndP (Binary (encrypted_loc, True), encrypt_network) in
  let decrypt_pkr = AndP (Binary (And (encrypted_loc, encrypt_pred), decrypt_pred), decrypt_network) in
  let preserve_pkr = AndP (Binary (And (Neg encrypted_loc, decrypt_pred), True), decrypt_network) in
  Comp (OrP (decrypt_pkr, preserve_pkr), Comp (routing_table, OrP (encrypt_pkr, preserve_pkr)))


let encrypt_network (table:Yojson.Basic.t) (man:man) (encrypted_list:string list): pkr =
  let routing_table = parse_global_routing_table table man in
  encrypt_network_from_routing_table routing_table man encrypted_list



let encrypt_packet_relation (man:man) (encrypted_list:string list): pkr =
  let encrypted_loc = parse_encrypted_loc_to_pred man encrypted_list in
  let encrypt_network = encrypt_to_pkr man 1 in
  let preserve_pkr = AndP (Binary (Neg encrypted_loc, True), Id) in
  let encrypt_pkr = AndP (Binary (encrypted_loc,True), encrypt_network) in
  OrP (preserve_pkr, encrypt_pkr)
...
let json_to_network (table:Yojson.Basic.t) (man:man) (dup_free:bool) (start_list:string list) (end_list:string list): NK.t =
  ...
  let entering_tunnel = encrypt_to_pred man 0 in
  let tunnel_filter = AndP (Binary (entering_tunnel, True), Id) in
  let open RN.NK in
  let network = 
    if dup_free then
      Star (Pkr routing_table)
    else
      Star (Seq (Pkr routing_table,Dup)) in
  Seq (Pkr (Comp (start_loc,tunnel_filter)), Seq (network,Pkr end_loc))

let encrypted_json_to_network (table:Yojson.Basic.t) (man:man) (dup_free:bool) (encrypted_list:string list) (start_list:string list) (end_list:string list): NK.t =
  let routing_table = encrypt_network table man encrypted_list in
  let start_loc = parse_start_loc_to_pkr man start_list in
  let end_loc = parse_end_loc_to_pkr man end_list in
  let entering_decrypt_tunnel = encrypt_to_pred man 0 in
  let entering_encrypt_tunnel = encrypt_to_pred man 1 in
  let encrypted_loc = parse_encrypted_loc_to_pred man encrypted_list in
  let tunnel_filter = OrP (AndP (Binary (And (entering_decrypt_tunnel,Neg encrypted_loc), True), Id),
                            AndP (Binary (And (entering_encrypt_tunnel,encrypted_loc), True), Id)) in
  let open RN.NK in
  let network = 
    if dup_free then
      Star (Pkr routing_table)
...
let parse_src_ip_filter_list (ips:string list) (man:man): pred =
  List.fold_left (fun acc ip -> Or (acc, parse_src_ip_filter ip man)) False ips  

let parse_dst_ip_filter (ip:string) (man:man): pred =
  let (ip, mask) = parse_ip_wildcard ip in
  binary_to_pred (header_placement DstIp man) mask 31 ip

let parse_dst_ip_filter_list (ips:string list) (man:man): pred =
  List.fold_left (fun acc ip -> Or (acc, parse_dst_ip_filter ip man)) False ips
...
let init_rela_man (rela:Yojson.Basic.t) : man =
  ...
  let interfaces_to_tunnels = (DStringMap.empty, DStringMap.empty) in (* No tunnels in RELA *)
  let tunnel_length = 0 in (* No tunnels in RELA *)
...
let rec parse_path_to_nk (rela_man:man) (path:path): NK.t =
  match path with
  | PSymbol s ->
      let loc = parse_rela_location_to_pkr s rela_man in
      Seq (Pkr loc, Dup)
  | PNegSymbols ss ->
      let negated_paths = NegP (List.fold_left (fun acc s ->
        let loc = parse_rela_location_to_pkr s rela_man in
        OrP (acc,loc)) Empty ss) in
      Seq (Pkr negated_paths, Dup)
  | PEmptySet ->
      Pkr Empty
  | PEpsilon ->
      Pkr Id
  | PUnion (p1, p2) ->
      ...
      let nk2 = parse_path_to_nk rela_man p2 in
      Union (SNK.add nk1 (SNK.singleton nk2))
  | PConcat (p1, p2) ->
      let nk1 = parse_path_to_nk rela_man p1 in
      let nk2 = parse_path_to_nk rela_man p2 in
      Seq (nk1, nk2)
