...
let header_placement (header:header) (man: man) : int =
  ...
  | SrcPorts -> 64 + (length_of_int (StringMap.cardinal man.nodes))
...
let partial_header_placement (header:header) (nodesmap: int StringMap.t) : int =
  ...
  | SrcPorts -> 64 + (length_of_int (StringMap.cardinal nodesmap))
...
let partial_parse_protocols_to_pred (protocols:string list) (nodesmap:int StringMap.t) : pred =
  let rec aux acc = function
    | [] -> acc
    | x::xs ->
        let id = StringMap.find x nodesmap in
        let protocol_length = length_of_int (StringMap.cardinal nodesmap) in
        aux (Or (acc, binary_to_pred (partial_header_placement Protocol nodesmap) protocol_length (protocol_length-1) id)) xs
  in
  if List.length protocols = 0 then
    True
  else
...
let rec parse_protocol_filter (node:string) (named_structure:string) (protocols:Yojson.Basic.t) (nodesmap:int StringMap.t) (in_filter:bool): pred =
      ...
      if (String.equal structure_type "IP_Access_List") && (String.equal structure_name named_structure) && (String.equal structure_node node) then
        ...
        let rec aux (acc:pred) = function
          ...
          | `List (x::xs) ->
              ...
              let dst_pred = binary_to_pred (partial_header_placement (if in_filter then SrcIp else DstIp) nodesmap) dst_mask 31 dstip in
              ...
              let src_pred = binary_to_pred (partial_header_placement (if in_filter then DstIp else SrcIp) nodesmap) src_mask 31 srcip in
              let protocol_list = [x] |> filter_member "matchCondition" |> filter_member "headerSpace" |> filter_member "ipProtocols" |> filter_string  in
              let protocol_pred = partial_parse_protocols_to_pred protocol_list nodesmap in
              let dstports_pred =
                try
                  parse_ports (x |> member "matchCondition" |> member "headerSpace" |> member "dstPorts") nodesmap (not in_filter)
                with _ -> True in
              let srcports_pred =
                try
                  parse_ports (x |> member "matchCondition" |> member "headerSpace" |> member "srcPorts") nodesmap in_filter
                with _ -> True in
              (* Construct the new filter *)  
              let new_filter =  (And (And (And (src_pred, srcports_pred), (And (dst_pred, dstports_pred))), protocol_pred)) in
              (Or (And (Neg acc,new_filter), aux (Or (acc, new_filter)) (`List xs)))
          | _ -> failwith "Unexpected JSON format for protocols, please check the json generated by the batfish is in orient format of 'record'" 
        in
        aux False local_entries
      else
        parse_protocol_filter node named_structure (`Assoc xs) nodesmap in_filter
...
let init_man (nodes:Yojson.Basic.t) (edges:Yojson.Basic.t) (protocols:Yojson.Basic.t) (interfaces:Yojson.Basic.t): man =
  let nodesmap = parse_nodes_to_map nodes in
  {
    ...
    interface = DStringMap.mapi 
      (fun (node,_) (in_filter, out_filter) -> (parse_protocol_filter node in_filter protocols nodesmap true, parse_protocol_filter node out_filter protocols nodesmap false)
